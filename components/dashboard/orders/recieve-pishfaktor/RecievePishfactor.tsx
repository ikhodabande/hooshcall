...
export default function RecievePishfactor({}: Props) {
  return (
    <div className="p-4 space-y-4">
      <PageTitles
        breadcrumbs={[
          ...
          { label: 'دریافت پیش فاکتور', href: '/dashboard/visitors/table' },
        ]}
            ...
            <TableBody className="text-xs md:text-sm">
              {Array.from({ length: 15 }).map((_, i) => (
                <TableRow key={i} className={cn(i % 2 === 0 ? 'bg-white' : 'bg-muted/50')}>
                  <TableCell className="hidden md:table-cell font-DanaNum">{(i % 3) + 1}</TableCell>
