...
const VisitorStatusBadge = ({ status }: { status: string }) => {
  const statusConfig = {
    pending: { label: 'در انتظار پردازش', variant: 'outline' },
    processing: { label: 'در حال پردازش', variant: 'outline' },
    shipped: { label: 'ارسال شده', variant: 'success' },
  ...
  return (
    <Badge className="rounded-[5px]" variant={config.variant as any}>
      {config.label}
    </Badge>
  );
};

export default function VisitorSettingModal({ order }: { order: any }) {
  return (
    <Dialog>
